...
const char* error_node::team_name(Status T1)
{
  ...
  case HASHNOTFOUND:
    return "Hashtable Not Found";
    
  case HASHTBLERROR:
    return "Hashtable Error";
                                 ...
                                 int lineno, const char *file, int error_index )
{
        ...
        extra = new char[strlen(file) + 10];
    sprintf( extra, "%s:%d", file, lineno );
    Status      status = add_error( new error_node(subsystem,priorStatus,error_index,extra) );
